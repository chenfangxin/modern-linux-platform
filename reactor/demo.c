...
/*
 * libev default loop, with an accept_watcher to accept the new connect
...
static void async_cb(EV_P_ ev_async *w, int revents)
{
	...
	if(NULL!=item){
		...
		printf("thread[%lu] accept: fd:%d addr:%s port:%d\n",
					((WORK_THREAD *)(w->data))->thread_id,
					item->sfd, item->szAddr,item->port);
...
static void setup_thread(WORK_THREAD *me)
{
	...
	/* Listen for notification from other threads */
...
void thread_init(void)
{
	...
	if(NULL==work_threads){
		perror("Can't allocate thread descriptiors\n");
		exit(1);
	}
	...
	/*
	 * Create threads after we've done all the libev setup */
	for(i=0;i<nthreads;i++){
		create_worker(worker_libev, &work_threads[i]);
	}

	/* Wait all the threads to set themseleves up befor returning. */
	pthread_mutex_lock(&init_lock);
...
void dispatch_conn(int anewfd, struct sockaddr_in asin)
{
	...
	// libev default loop, accept the new connection, round-robin dispatch to a work_thread.
...
loop:
	...
	if(ret>0){
		...
		printf("thread[%lu] remote socket closed! socket id:%d\n", pthread_self(), w->fd);
		close(w->fd);
		ev_io_stop(loop, w);
		free(w);
		return;
