...
#define EV_FEATURE_CODE     ((EV_FEATURES) &  0x1)
#define EV_FEATURE_DATA     ((EV_FEATURES) &  0x2)
#define EV_FEATURE_CONFIG   ((EV_FEATURES) &  0x4)
#define EV_FEATURE_API      ((EV_FEATURES) &  0x8)
#define EV_FEATURE_WATCHERS ((EV_FEATURES) & 0x10)
#define EV_FEATURE_BACKENDS ((EV_FEATURES) & 0x20)
#define EV_FEATURE_OS       ((EV_FEATURES) & 0x40)
...
extern ev_tstamp ev_now (struct ev_loop *loop)  ; /* time w.r.t. timers and the eventloop, updated after each poll */
...
# if EV_FEATURE_API
extern unsigned int ev_iteration (struct ev_loop *loop)  ; /* number of loop iterations */
extern unsigned int ev_depth     (struct ev_loop *loop)  ; /* #ev_loop enters - #ev_loop leaves */
extern void         ev_verify    (struct ev_loop *loop)  ; /* abort if loop data corrupted */

extern void ev_set_io_collect_interval (struct ev_loop *loop, ev_tstamp interval)  ; /* sleep at least this time, default 0 */
extern void ev_set_timeout_collect_interval (struct ev_loop *loop, ev_tstamp interval)  ; /* sleep at least this time, default 0 */
...
extern unsigned int ev_pending_count (struct ev_loop *loop)  ; /* number of pending events, if any */
