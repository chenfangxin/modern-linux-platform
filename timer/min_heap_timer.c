 ...
 * 2）每个节点的左子树和右子树都是一个堆
 *
 * 堆存储在数组中(因此堆的规模有一定限制)，利用数组下标快速访问堆中元素。
 * 节点i(i>=0)的父节点下标为(i-1)/2; 左右子节点的下标分别为(2*i+1)和(2*i+2)
 *
 * 插入操作：
 *   1) 将元素放在堆数组的尾部
 ...
 * 代码来自Libevent库
 *
 * 相对于二叉堆，四叉堆代码更复杂，但是更Cache友好，执行效率更高， libev库中可选四叉堆实现timer。
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/*
 * 用hole_index节点的父节点取代hole_index的位置，直到堆顶。
 * 看上去就像hole在向上走，走到合适位置后，将键值放在hole_index的位置
 * */
static void min_heap_shift_up(min_heap_t *s, uint32_t hole_index, struct rte_timer *tim)
{
	uint32_t parent = (hole_index>0)?((hole_index-1)/2):0;
...
/*
 * 用hole_index的子节点取代hole_index的位置，直到堆结束或要插入的键值大于hole_index的子节点。
 * 看上去像hole在向下走，走到合适位置后，将键值放在hole_index的位置
 * */
...
int del_min_heap_timer(struct rte_timer *tim)
{
	...
	if(-1!=tim->min_heap_idx){
		last = s->p[--s->n];
		s->p[s->n] = NULL;
		uint32_t parent = (tim->min_heap_idx>0)?((tim->min_heap_idx-1)/2):0;
